...
const Cart = () => {
    ...
    if(data.length===0){
        return(
            <div>
                <div className='m-5 w-100 text-center fs-3 text-white'>This Cart is Empty!</div>
            </div>
        )
    }
      ...
      <div className="container m-auto mt-5 table-responsive table-responsive-sm table-responsive-md">
        <table className='table table-hover'>
            ...
            <tbody className='text-white'>
                {data.map((food,index)=>(
                    <tr key={index} style={{ color: 'white' }} onMouseOver={(e) => e.target.style.color = 'red'} onMouseOut={(e) => e.target.style.color = 'white'}>
